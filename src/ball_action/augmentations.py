...
def get_train_augmentations(size: tuple[int, int]) -> nn.Module:
    ...
    transforms = nn.Sequential(
        ...
        augm.RandomRotation(degrees=(-3, 3), p=0.5),
        augm.RandomResizedCrop(size, scale=(0.8, 1.0), ratio=(ratio - 0.2, ratio + 0.2), p=0.9),
        augm.RandomHorizontalFlip(p=0.5),
        augm.RandomSharpness(sharpness=1., p=0.4),
        augm.RandomMotionBlur(kernel_size=11, angle=7.5, direction=1.0, p=0.4),
        augm.RandomBrightness(brightness=(0.8, 1.2), p=0.5),
        augm.RandomContrast(contrast=(0.8, 1.2), p=0.5),
        augm.RandomPosterize(bits=3, p=0.4),
        augm.RandomErasing(scale=(0.03, 0.06), ratio=(0.5, 2.0), p=0.5),
        augm.RandomGaussianNoise(mean=0., std=0.05, p=0.4),
    )
    return transforms
